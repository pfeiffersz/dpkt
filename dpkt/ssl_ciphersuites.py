...
class CipherSuite(object):
    ...
    * kx_auth: kx+auth algorithm, as 'KeyExchangeAlgorithm' in RFCs
    ...
    @property
    def aead(self):
        return ((self.mode == 'POLY1305' and self.cipher == 'CHACHA20') or
                (self.mode in ('CCM', 'CCM_8', 'GCM') and self.cipher.startswith('AES')))
...
class TestCipherSuites(object):
    ...
    def test_aead(self):
        ...
        assert (BY_NAME('TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384').aead == True)
        assert (BY_NAME('TLS_ECDHE_ECDSA_WITH_ARIA_256_GCM_SHA384').aead == False)

        assert (BY_NAME('TLS_DHE_RSA_WITH_AES_256_CCM').aead == True)
        assert (BY_NAME('TLS_DHE_RSA_WITH_AES_256_CCM_8').aead == True)
        assert (BY_NAME('TLS_DHE_RSA_WITH_AES_256_GCM_SHA384').aead == True)
        assert (BY_NAME('TLS_DHE_RSA_WITH_AES_256_CBC_SHA').aead == False)
        assert (BY_NAME('TLS_DHE_RSA_WITH_ARIA_256_GCM_SHA384').aead == False)

        assert (BY_NAME('TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305_SHA256').aead == True)
